...
AS
    IF UPDATE (desc_estado)
		...
		END
	ELSE
		PRINT 'Operacion correcta de actualización de estado'
GO
...
AS
    ...
    IF (@rowsAffected >1)
		BEGIN
			...
			RAISERROR ('No se puede borrar multiples registros, est�s borrando %d !!!', 16,1,@rowsAffected) 
			ROLLBACK TRAN
		END
	ELSE
...
No se puede borrar multiples registros, est�s borrando 10 !!!
...
AS
	...
	BEGIN
		PRINT 'Se est� intentando actuar sobre la tabla dimensiones y no se puede borrar registros, solo se actualizan'
...
--Se est� intentando actuar sobre la tabla dimensiones y no se puede borrar registros, solo se actualizan
